...
def apply_scores(products: list[dict], all_prices: list[float], budget_priority: bool = False) -> list[dict]:
    valid_products = [p for p in products if "error" not in p]
    if valid_products:
        feature_sets = [set(p.get("aspect_sentiments", {}).keys()) for p in valid_products]
    ...
    for p in products:
        ...
        ps  = score_price(p.get("price"), all_prices, budget_priority=budget_priority)
        ...
        p["scoring"] = compute_total_score(ps, rs, ss, as_, rari_trust, budget_priority=budget_priority)
...
def analyze():
    ...
    urls = data.get("urls", [])
    budget_priority = bool(data.get("budget_priority", False))
    ...
    products = apply_scores(products, all_prices, budget_priority=budget_priority)
